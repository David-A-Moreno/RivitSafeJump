...
public class ResultsScreen : UIComponent
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0) && _isCountingFinalScore)
        {
            ...
            else if(_gameManager is LevelSystemGameManager)
                finalScore.text = "" + ((_gameManager as LevelSystemGameManager) as LevelSystemGameManager).Score;
        }
    }
    ...
    private void ShowResults()
    {
        ...
        {
            int oldRecord = (_gameManager as LevelSystemGameManager).GetCurrentRanking()[0];
            (_gameManager as LevelSystemGameManager).RecordScore();
            int top = (_gameManager as LevelSystemGameManager).GetCurrentRanking()[0];
            topScore.text = "Puntaje máximo: " + top;
            currentCoins.text = (_gameManager as LevelSystemGameManager).Coins + "";
            int score = (_gameManager as LevelSystemGameManager).Score;
            var standards = (_gameManager as LevelSystemGameManager).GetStandardsOfCurrentLevel();
            StartCoroutine(achievementStars.CheckScore(score, standards, oldRecord));
            int obtainedCoins = (_gameManager as LevelSystemGameManager).AddCoins();
            StartCoroutine(CountCoins(obtainedCoins, 0.01F));
            StartCoroutine(CountFinalScore(score, 0.01F));
        }
    }
