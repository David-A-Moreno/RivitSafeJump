...
public class manejadorTablero : MonoBehaviour
{
    ...
    public void LlenarLista(List<string[]> nivel, List<int> ver, int i, GameObject boton)
    {
        if ((i != 0) && (angulos.Any()))
        {
            int angulosPos = i >= angulos.Count ? angulos.Count - 1 : i;

            if ((ver.Contains(i)))
            {
                if ((boton.GetComponent<Image>().sprite == tuboL) || (boton.GetComponent<Image>().sprite == tuboCruz))
                {
                    FichasEspecialesQuitar(boton, angulos[angulosPos], i, ver);
                }else if (angulos[angulosPos] != boton.GetComponent<Giro>().angulo)
                    ver.Remove(i);
            }
            else if (!(ver.Contains(i)))
            {
                if((boton.GetComponent<Image>().sprite == tuboL) || (boton.GetComponent<Image>().sprite == tuboCruz))
                {
                    FichasEspecialesAñadir(boton, angulos[angulosPos], i, ver);
                }
                else if (angulos[angulosPos] == boton.GetComponent<Giro>().angulo)
                {
                    ver.Add(i);
